...
void MeshLoader::SaveCurrentMesh()
{
	string fileName = to_string(currentSave).append(".obj");
	currentSave++;

	ofstream file(fileName);

	file << "# obj file generated by MeshDeformation program." << endl;
	for (int i = 0; i < currentVertices.size(); i++)
	{
		file << "v " << currentVertices[i].x << " " << currentVertices[i].y << " " << currentVertices[i].z << endl;
	}
	for (unsigned int i = 0; i< vertexIndices.size(); i+= 3)
	{
		file << "f " << vertexIndices[i] + 1 << " " << vertexIndices[i + 1] + 1 << " " << vertexIndices[i + 2] + 1 << endl;
	}
